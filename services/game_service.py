...
# --- WEB GAMES (NEW) ---
def save_web_game_score(user_id: int | str, game_type: str, score: int, difficulty: str = None) -> bool:
    """Web oyunları için skor kaydeder."""
    if not supabase: return False
    try:
        # Önce RPC fonksiyonunu dene (daha güvenli ve atomic olabilir)
        try:
            supabase.rpc("save_game_score", {
...
def get_web_game_high_score(user_id: int | str, game_type: str) -> int:
    ...
    try:
        # RPC dene
        try:
            response = supabase.rpc("get_high_score", {
                "p_user_id": str(user_id),
                "p_game_type": game_type
            }).execute()
